-- Data cleaning with SQL

-- USE layoffs_db ;


SELECT * 
FROM layoffs ;

-- 1. Remove Duplicates 
-- 2. Standardize the Data
-- 3. Null and blanks values
-- 4. Remove any columns 

-- Copie du table layoffs 

CREATE TABLE layoffs_copie
LIKE layoffs;

SELECT * 
FROM layoffs_copie ;

-- Inserer les données dans la table layoffs_copie

INSERT layoffs_copie
SELECT *
FROM layoffs;

SELECT * ,
ROW_NUMBER() OVER(
PARTITION BY company, location, industry, total_laid_off, percentage_laid_off, `date`) AS row_num
FROM layoffs_copie;


WITH duplicate_cte AS 
(
SELECT * ,
ROW_NUMBER() OVER(
PARTITION BY company, location, industry, total_laid_off, percentage_laid_off, `date`,
stage, country, funds_raised_millions) AS row_num
FROM layoffs_copie
)
SELECT *
FROM duplicate_cte
WHERE row_num > 1 ;

-- 1. Supprimer la table si elle existe déjà

DROP TABLE IF EXISTS layoffs_copie2;

CREATE TABLE `layoffs_copie2` (
  `company` text,
  `location` text,
  `industry` text,
  `total_laid_off` int DEFAULT NULL,
  `percentage_laid_off` text,
  `date` text,
  `stage` text,
  `country` text,
  `funds_raised_millions` int DEFAULT NULL, 
  `row_num`  INT 
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;


INSERT INTO layoffs_copie2
SELECT * ,
ROW_NUMBER() OVER(
PARTITION BY company, location, industry, total_laid_off, percentage_laid_off, `date`,
stage, country, funds_raised_millions) AS row_num
FROM layoffs_copie;


SELECT *
FROM layoffs_copie2
WHERE row_num > 1;


DELETE
FROM layoffs_copie2
WHERE row_num > 1;

SELECT *
FROM layoffs_copie2
WHERE row_num > 1;

-- STANDARDIZING DATA

SELECT company, TRIM(company)
FROM layoffs_copie2;


UPDATE layoffs_copie2
SET company = TRIM(company);

SELECT DISTINCT industry
FROM layoffs_copie2
ORDER BY 1;

SELECT *
FROM layoffs_copie2
WHERE industry LIKE 'Crypto%';

UPDATE layoffs_copie2
SET industry = 'Crypto'
WHERE industry LIKE 'Crypto%';

SELECT DISTINCT industry
FROM layoffs_copie2;

SELECT DISTINCT country
FROM layoffs_copie2
ORDER BY 1;

SELECT *
FROM layoffs_copie2
WHERE country LIKE 'United States%'
ORDER BY 1;

SELECT DISTINCT country, TRIM(TRAILING '.' FROM country)
FROM layoffs_copie2
ORDER BY 1;



UPDATE layoffs_copie2
SET  country = TRIM(TRAILING '.' FROM country)
WHERE country LIKE 'United States%';


SELECT `date`
FROM layoffs_copie2;


UPDATE layoffs_copie2
SET `date` = STR_TO_DATE(`date`, '%m/%d/%Y');


ALTER TABLE layoffs_copie2
MODIFY COLUMN `date` DATE;

-- Null and blanks values

SELECT * 
FROM layoffs_copie2
WHERE industry IS NULL 
or industry = '';

SELECT * 
FROM layoffs_copie2
WHERE company = 'Airbnb';

UPDATE layoffs_copie2
SET industry = NULL
WHERE industry = '';

SELECT  t1.industry, t2.industry
FROM layoffs_copie2 t1
JOIN layoffs_copie2 t2
ON t1.company = t2.company
AND t1.location = t2.location
WHERE (t1.industry IS NULL OR t1.industry = '')
AND t2.industry IS NOT NULL  ;

UPDATE  layoffs_copie2 t1
JOIN layoffs_copie2 t2
    ON t1.company = t2.company
SET t1.industry = t2.industry
WHERE t1.industry IS NULL
AND t2.industry IS NOT NULL ;

SELECT *
FROM layoffs_copie2
WHERE total_laid_off IS NULL
AND percentage_laid_off IS NULL;

DELETE 
FROM layoffs_copie2
WHERE total_laid_off IS NULL
AND percentage_laid_off IS NULL;


SELECT *
FROM layoffs_copie2;

ALTER TABLE layoffs_copie2
DROP COLUMN row_num;